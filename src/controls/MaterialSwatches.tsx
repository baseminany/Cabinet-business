...
export function swatchBackground(m: MaterialDef): string {
  if (m.kind === 'wood') return `linear-gradient(160deg, ${shade(m.color, -0.04)} 0%, ${m.color} 45%, ${shade(m.color, 0.18)} 100%)`;
...
export default function MaterialSwatches({ label, value, onChange, role = 'carcass' }: { label: string; value: MaterialId; onChange: (id: MaterialId) => void; role?: MaterialRole }) {
  ...
  const groups = Array.from(new Set(options.map((m) => m.category))).map((category) => ({ category, items: options.filter((m) => m.category === category) }));
  ...
  return (
    <div>
      ...
      <div className="space-y-4">
        {groups.map((g) => (
          <div key={g.category}>
            <p className="mb-2 text-[10px] font-black uppercase tracking-[0.16em] text-brass">{g.category}</p>
            <div className="grid grid-cols-2 gap-2">
              {g.items.map((m) => {
                const active = m.id === value;
                return (
                  <button key={m.id} type="button" title={m.label} onClick={() => onChange(m.id)} className={'group overflow-hidden rounded-2xl border bg-warmWhite p-2 text-left transition active:scale-[0.99] ' + (active ? 'border-brass shadow-premiumGlow' : 'border-champagne/25 hover:border-brass/60 hover:shadow-soft')}>
                    ...
                    {m.code && <div className="mt-0.5 text-[10px] font-bold text-brass">{m.code}</div>}
