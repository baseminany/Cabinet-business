  ...
  return (
    <div className="nice-scroll min-h-0 flex-1 overflow-y-auto bg-[linear-gradient(180deg,#fffdf8,#f4eadb)] px-6 py-8 text-ink sm:px-10">
      <div className="mx-auto flex max-w-6xl items-center justify-between">
        <button onClick={() => setStep('welcome')} className="text-[11px] font-bold uppercase tracking-[0.18em] text-ink-muted transition hover:text-brass">← Studio</button>
        <span className="rounded-full border border-champagne/40 bg-white/70 px-3 py-1.5 text-[10px] font-bold uppercase tracking-[0.18em] text-ink-muted">Setup</span>
      </div>

      <div className="mx-auto grid max-w-6xl items-end gap-10 pb-12 pt-12 lg:grid-cols-[0.9fr_1.1fr] lg:pt-20">
        <div className="fade-up">
          <p className="eyebrow text-brass">Start however feels easiest</p>
          <h1 className="mt-5 text-5xl font-semibold leading-[0.94] tracking-[-0.05em] text-ink sm:text-7xl">How should we map your space?</h1>
          <p className="mt-6 max-w-md text-base leading-8 text-ink-soft">Use a photo as reference, build a blank room, or design one piece. Nothing is locked in.</p>
        </div>
        ...
        <div className="grid gap-4 md:grid-cols-3">
          <div onDragOver={(e) => { e.preventDefault(); setDragOver(true); }} onDragLeave={() => setDragOver(false)} onDrop={(e) => { e.preventDefault(); setDragOver(false); handleFile(e.dataTransfer.files?.[0]); }} className={'premium-card relative flex min-h-[22rem] flex-col p-6 text-ink transition ' + (dragOver ? 'ring-2 ring-brass' : '')}>
            ...
            <h2 className="mt-5 text-2xl font-black tracking-tight">Upload a room photo</h2>
            <p className="mt-2 flex-1 text-sm leading-6 text-ink-muted">Use a photo for reference. If an analysis endpoint is connected, Studio can create a draft; otherwise you will draw the room manually.</p>
          ...
          <EntryCard icon={<svg width="26" height="26" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.6"><path d="M3 10.5 12 4l9 6.5V20a1 1 0 0 1-1 1H4a1 1 0 0 1-1-1z" /><path d="M9 21v-6h6v6" /></svg>} title="Build a blank room" body="Start with an empty room shape, then add windows, doors, and cabinetry." cta="Start blank" onClick={startBlankRoom} />

          <EntryCard icon={<svg width="26" height="26" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.6"><rect x="5" y="3" width="14" height="18" rx="1.5" /><line x1="12" y1="3" x2="12" y2="21" /></svg>} title="Design a piece" body="Skip the room and create a cabinet, shelf, pantry, or mudroom piece by size." cta="Start with a piece" onClick={() => { setRoom({ enabled: false, openings: [] }); setStep('pieces'); }} />
        </div>
      </div>
    </div>
  );
